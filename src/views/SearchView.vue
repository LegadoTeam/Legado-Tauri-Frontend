...
const sourceTypeOptions = computed(() => {
  const counts = allSearchableSources.value.reduce(
    ...
    { all: 0, novel: 0, comic: 0, video: 0, music: 0 } satisfies Record<
      SearchSourceType,
      number
    >,
  );
...
const searchableSources = computed(() => {
  ...
  return allSearchableSources.value.filter(
    (source) =>
      normalizeSourceType(source.sourceType) === selectedSearchType.value,
  );
});
...
const selectedTypeLabel = computed(
  () => sourceTypeLabels[selectedSearchType.value],
);
...
const aggregatedEmptyDescription = computed(() =>
  limitedSource.value
    ? `${limitedSourceName.value} 暂无搜索结果`
    : "当前搜索范围暂无结果",
);
...
async function loadSources() {
  try {
    ...
    message.error(
      `加载书源列表失败: ${e instanceof Error ? e.message : String(e)}`,
    );
  }
}
...
watch(searchableSources, (nextSources) => {
  ...
  const found = nextSources.find(
    (source) => source.fileName === limitedSource.value?.fileName,
  );
  if (found) {
    limitedSource.value = found;
    return;
...
const completedSourceCount = computed(
  () =>
    activeSources.value.filter(
      (s) => searchStates[s.fileName] && !searchStates[s.fileName].loading,
    ).length,
);
...
const totalRawResultCount = computed(() =>
  activeSources.value.reduce(
    (sum, s) => sum + (searchStates[s.fileName]?.results.length ?? 0),
    0,
  ),
);
...
const sourcesWithResultCount = computed(
  () =>
    activeSources.value.filter(
      (s) => (searchStates[s.fileName]?.results.length ?? 0) > 0,
    ).length,
);
    ...
    await mapWithConcurrencyLimit(
      ...
      async (src) => {
        try {
          ...
          searchStates[src.fileName].results = Array.isArray(raw)
            ? (raw as BookItem[])
            : [];
        } catch (e: unknown) {
          if (requestToken !== activeSearchToken.value) {
          ...
          searchStates[src.fileName].error =
  ...
  <div class="search-view" :style="searchDensityStyle">
    <AppPageHeader title="搜索">
      ...
      <template #subtitle>
        ...
        <template v-else>
          找到
          <span class="sv-header__count">{{ totalRawResultCount }}</span>
          条结果 · {{ sourcesWithResultCount }} /
          {{ activeSources.length }} 个书源有结果
        </template>
      </template>
      <template #actions>
        <MobileToolbarMenu
        ...
        >
          ...
          <n-dropdown
            trigger="click"
            :options="
              CARD_SIZES.map((size) => ({ label: size.label, key: size.key }))
            "
            :value="activeSizeKey"
    ...
    <div class="sv-content">
      ...
      <div v-if="hasSearched && !searchRunning" class="sv-pagination">
        ...
        <button class="sv-page-btn" @click="goToSearchPage(searchPage + 1)">
          下一页
        </button>
      </div>
    </div>
